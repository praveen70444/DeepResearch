...
const DocumentUpload: React.FC = () => {
  ...
  const getFileIcon = (file: File) => {
    ...
    if (type.includes('pdf') || extension === 'pdf') return <File className="w-5 h-5 text-red-500" />;
    if (type.includes('word') || type.includes('document') || ['doc', 'docx'].includes(extension || '')) return <FileText className="w-5 h-5 text-blue-500" />;
    if (type.includes('image') || ['jpg', 'jpeg', 'png', 'gif', 'bmp'].includes(extension || '')) return <Image className="w-5 h-5 text-green-500" />;
    if (type.includes('sheet') || type.includes('excel') || ['xls', 'xlsx', 'csv'].includes(extension || '')) return <FileSpreadsheet className="w-5 h-5 text-green-600" />;
    if (['txt', 'md', 'markdown'].includes(extension || '')) return <FileText className="w-5 h-5 text-gray-600" />;
    if (['html', 'htm'].includes(extension || '')) return <FileText className="w-5 h-5 text-orange-500" />;
    if (['json'].includes(extension || '')) return <FileText className="w-5 h-5 text-yellow-500" />;
